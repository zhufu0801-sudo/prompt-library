...
type Scenario = (typeof originalScenarios)[number] & {
  source?: (typeof curatedScenarios)[number]['source'];
};
...
export function moduleGroup(t: Template) {
  return t.id.startsWith('studio-')
    ? t.id.slice(7)
    : (
        {
...
function visibleGuides(t: Template) {
  return guides.filter(
    (g) =>
      g.group === moduleGroup(t) &&
      (t.id === 'custom-image'
        ? g.id === 'image'
        : t.id === 'custom-animation'
          ? g.id !== 'image'
          : true),
  );
}
export function taskTemplates(templates: Template[], locale: Locale) {
  return templates
    .filter((t) => studioIds.includes(t.id))
    .flatMap((template) =>
      visibleGuides(template).map((g) => ({
        id: g.id,
        label: g.labels[locale],
        description: g.guidance[locale],
        template,
...
export function composeStudio(
  ...
  const scope = {
    zh: '以下基础规范只适用与当前任务直接相关的部分；以所选场景的交付物为准，不为单元测试等任务额外创建界面或接口。',
    en: 'Apply base rules only where relevant to this task. Use the selected scenario’s deliverables; do not create unrelated UI or APIs for tasks such as unit tests.',
    ja: '基本要件は今回のタスクに関係する部分だけ適用し、選択シナリオの成果物を優先する。単体テストなどに不要な画面やAPIを追加しない。',
  }[locale];
  const guide =
    (matches.length ? scope + '\n\n' : '') +
    baseGuide +
    (matches.length
      ? '\n\n' +
        matches
          .map((x) => x.labels[locale] + ': ' + x.details[locale])
          .join('\n\n')
      : '') +
    '\n\n' +
    adapter.labels[locale] +
    ': ' +
    adapter.guidance[locale];
  const text = (meta ? sections.join('\n\n') : sections[0])
    .replace('[[TASK_GUIDE]]', guide)
  ...
  return (
    ...
    {
      zh: 'Skill 辅助说明',
      en: 'Skill-assisted instructions',
      ja: 'Skill 補助指示',
    }[locale] +
    ': ' +
    skill.labels[locale] +
      ...
      return [
        {
          ...s,
          task: target.id,
          taskLabel: target.labels[locale],
          currentTask: target.id === current.id,
          scope: skillFit[s.id as keyof typeof skillFit]?.scope[locale] || '',
        },
      ];
    })
...
export function applySkillRecommendation(
  ...
  if (!skill || locks.skill_id || (!skill.currentTask && locks[taskKey(t)]))
    return values;
  return {
    ...values,
    skill_id: id,
    ...(!skill.currentTask
      ? {
          [taskKey(t)]: skill.taskLabel,
          ...(!locks.scenario
            ? {
                scenario:
                  t.fields.find((f) => f.key === 'scenario')?.options[0] || '',
              }
            : {}),
        }
      : {}),
  ...
  return Object.fromEntries(
    Object.entries(values).map(([key, value]) => {
      ...
      const translate = (v: string) => {
        ...
        const targetLabel = to.fields.find((f) => f.key === taskKey(to))
          ?.options[0];
        const firstGuide = guides.find((g) =>
          Object.values(g.labels).includes(targetLabel || ''),
        )!;
        const targetLocale =
          firstGuide &&
          (Object.keys(firstGuide.labels) as Locale[]).find(
            (l) => firstGuide.labels[l] === targetLabel,
          );
        if (targetLocale) {
          ...
          for (const sourceLocale of ['zh', 'en', 'ja'] as Locale[]) {
            const sourceField =
              localized[sourceLocale][key as keyof typeof localized.zh];
            const targetField =
              localized[targetLocale][key as keyof typeof localized.zh];
            if (
              sourceField &&
              targetField &&
              'options' in sourceField &&
              'options' in targetField
            ) {
              const optionIndex = (sourceField.options as string[]).indexOf(v);
              if (optionIndex >= 0)
                return targetField.options[optionIndex] ?? v;
            }
          }
        }
        return v;
...
function taskGuide(t: Template, values: Values) {
  ...
  const selected = guides.find(
    (g) => g.group === group && Object.values(g.labels).includes(selection),
  );
  const fallback = ['AI 动画', 'AI animation', 'AIアニメーション'].includes(
    selection,
  )
    ? baseGuides.find((g) => g.id === 'storyboard')!
    : visibleGuides(t)[0] || baseGuides[0];
...
export function taskFields(
  ...
  const overrides = (inputs[guide.id as keyof typeof inputs]?.[locale] ||
    {}) as Record<string, Partial<Field> & { placeholder?: string }>;
  ...
  const selected = [
    ...
    (s) =>
      s.task === current &&
      Object.values(s.labels).includes(String(values.scenario || '')),
  );
  return t.fields
    .filter((f) => f.key !== 'skill_id')
    .map((f) => {
      const field = { ...f, ...overrides[f.key] };
      if (extension && f.key === 'subject') {
        field.options = [extension.examples[locale]];
        field.placeholder = extension.examples[locale];
      }
      if (extension && f.key === 'materials')
        field.placeholder = extension.questions[locale];
      if (f.key === 'scenario')
        field.options = [
          f.options[0],
          ...scenarios
            .filter((s) => s.task === current)
            .map((s) => s.labels[locale]),
        ];
...
export function toolAdapter(t: Template, values: Values) {
  const group = moduleGroup(t);
  if (!['programming', 'visual'].includes(group))
    return {
      id: 'general-chat',
      labels: { zh: '通用对话AI', en: 'General chat AI', ja: '汎用対話AI' },
      guidance: {
        zh: '按目标软件和版本提供内容；没有文件操作能力时交付可复制正文及步骤，不声称已制作或修改文件。',
        en: 'Match the target software/version. Without file tools, provide copyable content and steps rather than claiming file creation or edits.',
        ja: '対象ソフトと版に合わせる。ファイル操作機能がなければ貼付用本文と手順を出し、制作・修正済みとしない。',
      },
    };
  const options = adapters.filter((a) => a.group === group);
  return (
    options.find((a) =>
      Object.values(a.labels).includes(String(values.tool || '')),
    ) || options[0]
  );
}
...
export function analyzeTask(t: Template, values: Values, locale: Locale) {
  ...
  const ranked = subject
    ? scenarios
        .map((s) => ({ ...s, ...scenarioEvidence(subject, s.id, s.terms) }))
        .filter((s) => s.score >= 2)
          ...
          evidence: [
            ...new Set([
              ...matchedTerms(
                subject,
                intentTerms[g.id] ||
                  extendedTasks.find((x) => x.id === g.id)?.terms ||
                  [],
              ),
              ...ranked
                .filter((s) => s.task === g.id)
                .slice(0, 1)
    ...
    ? [
        {
          ...selected,
          evidence: [
            { zh: '手动选择', en: 'Selected by you', ja: '手動選択' }[locale],
          ],
        },
      ]
    : ranked.filter((s) => s.task === current.id).slice(0, 1);
  return {
    current: current.id,
    candidates,
    scenarios: matched,
    missing: subject
      ? ['materials', 'criteria', 'constraints'].filter(
...
export function briefQuestion(t: Template, values: Values, locale: Locale) {
  const guide = taskGuide(t, values),
    ext = extendedTasks.find((x) => x.id === guide.id);
  if (!String(values.subject || '').trim())
    return {
      key: 'subject',
      text: {
        zh: '先写一句话：你想完成什么？',
        en: 'Start with one sentence: what do you want to achieve?',
        ja: 'まず一文で、何をしたいか教えてください。',
      }[locale],
    };
  ...
  if (
    guide.id === 'debug' &&
    /```|traceback|stack trace|at \w+.*:\d+|报错[：:]\s*\S+/i.test(text)
  )
    return null;
  if (
    ['clip', 'storyboard'].includes(guide.id) &&
    /\d+\s*(秒|seconds?|sec|s\b)/i.test(text)
  )
    return null;
  if (ext && /资料[：:]|原文[：:]|data:|source:|資料[:：]/i.test(text))
    return null;
  const questions: Record<string, string[]> = {
    debug: [
      '请补充报错原文、相关代码和运行环境；已写在需求里就不用重复。',
      'Add the exact error, relevant code and environment; omit anything already supplied.',
      'エラー原文、関連コード、環境を補足。既出なら重複不要です。',
    ],
    image: [
      '有没有参考图？希望的画幅和需要保持不变的主体是什么？',
      'Do you have a reference image? What aspect ratio and subject must be preserved?',
      '参照画像はありますか？比率と固定する被写体を教えてください。',
    ],
    clip: [
      '这段视频多长？从参考图开始，还是先构思首帧？',
      'How long is the clip, and is there an input image or should we plan one?',
      '尺は何秒ですか？参照画像から始めますか？',
    ],
    storyboard: [
      '总时长、故事梗概和已有角色资料是什么？',
      'What are the total duration, story and existing character references?',
      '総尺、物語、人物資料を教えてください。',
    ],
  };
  const fallback = [
    '有哪些已有资料或不可改变的要求？',
    'What existing materials or fixed requirements should be used?',
    '既存資料や変更できない条件は何ですか？',
  ];
  return {
    key: 'materials',
    text: ext
      ? ext.questions[locale]
      : (questions[guide.id] || fallback)[['zh', 'en', 'ja'].indexOf(locale)],
  };
}
